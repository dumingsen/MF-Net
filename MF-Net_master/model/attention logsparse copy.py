...
#=====================================================================================================================
        ...
        print('q k1 v',q.shape,k1.shape,v.shape)#torch.Size([16, 3, 24, 32, 32]) torch.Size([16, 3, 24, 32, 32]) torch.Size([16, 3, 24, 32, 32])
...
#1
            ...
            print('qkv',qkv.shape)#3, 16, 3, 768, 64
            q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
            print('q k v',q.shape,k.shape,v.shape)#16, 3, 768, 64])

            print('prob x',x.shape)#([16, 768, 192]
            value = self.value(x)
            print('value1',value.shape)#[16, 768, 576])
            qk_x = nn.functional.pad(x.permute(0, 2, 1), pad=(self.q_len - 1, 0))
            print('qk_x',qk_x.shape)#[16, 192, 768]
            query_key = self.query_key(qk_x).permute(0, 2, 1)
            print('query_key',query_key.shape)#([16, 768, 1152])
            query, key = query_key.split(self.split_size, dim=2)
            print('q k',query.shape,key.shape)#[16, 768, 576]) torch.Size([16, 768, 576])
            query = self.split_heads(query)
            print('query',query.shape)#16, 3, 768, 192]
            key = self.split_heads(key, k=True)
            print('key',key.shape)#[16, 3, 192, 768]
            value = self.split_heads(value)
            print('value2',value.shape)#[16, 3, 768, 192]

# # 2 attention qk
#             scores_top, index = self._compute_q_k(query, key)
#             print('scores_top',scores_top.shape)#scores_top torch.Size([16, 3, 35, 32])
#             print('index',index.shape)#index torch.Size([16, 3, 35])
...
#
            ...
            #     print('relative_indices',self.relative_indices.shape)#[64, 64]
            #     print('pos_embedding',self.pos_embedding.shape)#pos_embedding torch.Size([127]#
            ...
            #     attn += self.pos_embedding[self.relative_indices[index].to(torch.long)]
            # else:
...
#3 attention attn
    ...
    def attn(self, query: torch.Tensor, key, value: torch.Tensor, activation="Softmax"):
        activation = activation_dict[activation](dim=-1)
        pre_att = torch.matmul(query, key)
        print('pre_att', pre_att.shape)  # torch.Size([16, 3, 768, 768])
        ...
        if self.relative_pos_embedding:  ##相对位置bias
            ...
            print('relative pos embedding pre_att', pre_att.shape)
        ...
        if self.scale1:
            pre_att = pre_att / math.sqrt(value.size(-1))
        print('pre_att = pre_att / math.sqrt(value.size(-1))',(pre_att / math.sqrt(value.size(-1))).shape)#torch.Size([16, 3, 768, 768])
        print('pre_att.size(-2)',pre_att.size(-2))#768
        mask = self.mask_tri[:, :, :pre_att.size(-2), :pre_att.size(-1)]
        print('mask',mask.shape)#([1, 1, 20, 20]
        pre_att = pre_att * mask + -1e9 * (1 - mask)#
        pre_att = activation(pre_att)#([16, 768, 192]
        pre_att = self.attn_dropout(pre_att)
        print('pre_att',pre_att.shape)
        attn = torch.matmul(pre_att, value)
        rint('attnn', attn.shape)
        return attn
    ...
    def _prob_QK1(self, Q, K, sample_k, n_top): # n_top: c*ln(L_q)##C
        # Q [B, H, L, D]
        B, H, L_K, E = K.shape
        _, _, L_Q, _ = Q.shape
    ...
    def _compute_q_k(self, q, k):#C
        print('k',k.shape)#[16, 3, 192, 768]
        print('q',q.shape)#[16, 3, 768, 192]
        B,H,L_Q,D = q.shape#
        _, _,L_K,_ = k.shape#
        # 如论文所述，这里的L_Q=L_K=L
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):##copy
        ...
        L_Q, sample_k))  # real U = U_part(factor*ln(L_k))*L_q 为每一个query采样sampe_k个key的index
        ...
        Q_K_sample = torch.matmul(Q.unsqueeze(-2), K_sample.transpose(-2, -1)).squeeze(
            -2)  # 这里的squeeze需要加上-2如果不加的话，在运行batch_size=1时会出现问题
    ...
    def __init__(self, t,dim, heads, head_dim, mlp_dim, shifted,
        ...
        self.attention_block = Residual(PreNorm(dim, WindowAttention(t,
            ...
            wa_dim=wa_dim,#wa_dim=down_dim // reduce(lambda x, y: x * y,downscaling_factors[:i + 1]),#(4,2, 2,2),  # 代表多长的时间作为一个特征
                                                                     ...
                                                                     prob=prob,
                                                                     mask=mask,
                                                                     win_len=768, scale1=False, q_len=1, sub_len=1, sparse=True, attn_pdrop=0.1, resid_pdrop=0.1,
                                                                     )))
        ##
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        print('PatchMerging ',x.shape)#torch.Size([16, 3072, 144])
        x = rearrange(x, 'b (p f) c -> b f (p c)', p=self.downscaling_factor)  # p c 是一个特征   f是代表分为了多少段
        print('PatchMerging ', x.shape)#([16, 768, 576])
        x = self.linear(x)#[16, 768, 192])
...
class attentionNet(nn.Module):
    ...
    def forward(self, ts):##ts为流入的训练数据
        ...
        x = ds.transpose(2, 1)
        print('transpose ds',x.shape)#torch.Size([16, 3072, 144])
