...
class gtnet(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None,
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Linear(t, num_classes)#hidden_dim[-1]
        )
        ...
        #self.gate = torch.nn.Linear(num_nodes*t + 48*t + 768*t , 3)#x y z
        self.gate = torch.nn.Linear(num_nodes + 48 + 768 , 3)  # x y z
        self.output_linear =nn.Sequential(
            nn.LayerNorm(num_nodes*t + 48*t + 768*t),  ##62,normalized_shape：归一化的维度，int（最后一维）list（list里面的维度）#hidden_dim[-1]
            torch.nn.Linear(num_nodes*t + 48*t + 768*t, num_classes)
        )
        #self.output_linear = torch.nn.Linear(num_nodes*t + 48*t + 768*t, num_classes)

        #self.cbam=cbamblock(num_nodes*t + 48*t + 768*t,16,7)
...
#图卷积=============================================================================================================
        ...
        x = self.start_conv(input)  # 二维卷积
        print('x', x.shape)  # torch.Size([4, 16, 137, 187])/torch.Size([4, 16, 144, 62])
        ...
        print('图卷积开始x', x.shape)  # torch.Size([4, 16, 137, 181])/ torch.Size([4, 16, 144, 62])
        ##图卷积,输入是上一层的x
        if self.gcn_true:  # 有图
        ...
        print(x.shape)#[16, 64, 144, 62])
        x = self.end_conv_2(x)  # 二维卷积
        x=torch.squeeze(x)
        print('图卷积',x.shape)#[16, 144, 62])#[16, 9, 144])
...
#全局特征自注意力机制=============================================================================================================
        ...
        print('自注意力机制',z.shape)##torch.Size([16, 192, 62])#16, 768, 62#[15, 768, 640]
##特征拼接层=============================================================================================================
        ...
        # 1筛选加权机制
        # gate = F.softmax(self.gate(torch.cat([x, y,z], dim=-1)), dim=-1)
        # print('gate',gate.shape)#[15, 3])
        # xyz = torch.cat([x * gate[:, 0:1], y * gate[:, 1:2],z * gate[:, 2:3]], dim=-1)
        gate = F.softmax(self.gate(torch.cat([x, y, z], dim=1)), dim=-1)
        print('gate', gate.shape)  # [15, 3])
        xyz = torch.cat([x * gate[:, 0:1], y * gate[:, 1:2], z * gate[:, 2:3]], dim=-1)
        print('gate[:, 0:1]',gate[:, 0:1].shape)#([15, 1]
        print('x * gate[:, 0:1]',(x * gate[:, 0:1]).shape)
        print('xyz', xyz.shape)#[16, 983934]
        #注意力
        #xyz=self.cbam(xyz)

        #2普通拼接
        #xyz = torch.cat((x, y, z), 1)  #
        print('特征拼接层 xyz',xyz.shape)#[16,384,62],#[16, 983934]
#分类层====================================================== =======================================================
        ##取通道平均值操作
